...
function MonitorGroup({ position, rotation, scale, nodes, materials }) {
  ...
  const handleHover = (hover) => {
    setIsHovered(hover);
    if (hover) {
      setAdditionalScreens(generateScreens());
    } else {
      setAdditionalScreens([]);
    }
  };

  // Function to generate additional screens in a circular layout
  const generateScreens = () => {
    const screenCount = 6; // Number of surrounding screens
    const radius = 1.5; // Radius of the circular arrangement
    const angleStep = (2 * Math.PI) / screenCount;
    ...
    return Array.from({ length: screenCount }, (_, i) => {
      ...
      return {
        position: [
          ...
          0.5, // Y-axis for slight height adjustment
  ...
  // Spring animation for scaling the surrounding screens
  const { scaleSpring } = useSpring({
    scaleSpring: isHovered ? 1 : 0, // Scale up when hovered, otherwise scale down
  ...
  return (
    ...
    >
      ...
      {/* Animated surrounding screens */}
      {additionalScreens.map((screen, index) => (
        <a.group
          position={screen.position}
          rotation={screen.rotation}
          key={index}
          scale={scaleSpring.to((s) => [s, s, s])} // Animated scale
        >
          <mesh
            castShadow
            receiveShadow
